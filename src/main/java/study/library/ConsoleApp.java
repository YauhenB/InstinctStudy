...
public class ConsoleApp {
    ...
    public static void run(final String dbType)
    {
        switch (dbType) {
            case "SQL": {
        ...
        while (flag) {
            ...
            switch (sc.nextInt()) {
                ...
                case 9: {
                    try {
                        System.out.println("Username:");
                        final User user = userDao.load(sc.next());
                        System.out.println("Book name:");
                        final Book book = bookDao.load(sc.next());
                        book.setOwner(user);
                        bookDao.create(book);
                    } catch (Exception e) {
                        System.err.println("WRONG INPUT");
